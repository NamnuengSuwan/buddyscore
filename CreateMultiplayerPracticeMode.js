...
export function CreateMultiplayerPracticeMode() {
  ...
  const handleCreateGame = () => {
    ...
    navigate('MultiplayerGamePracticeMode');
  };

  const filteredUsers1 = users.filter((user) => user !== selectedValue2);
  const filteredUsers2 = users.filter((user) => user !== selectedValue1);
  const filteredUsers3 = users.filter((user) => user !== selectedValue3);
  const filteredUsers4 = users.filter((user) => user !== selectedValue4);

  const renderHeader = () => {
    return (
      <View style={[styles.header, styles.shadow]}>
        <Text style={styles.headerTitle}>{'หน้าสร้างเกมคู่โหมดฝึกซ้อม'}</Text>
      </View>
    );
  };
  return (
  ...
  <ScrollView>
    <View style={styles.container}>
      ...
      <Text style={styles.titleText}>ชื่อทีม </Text>
      <TextInput
        style={styles.input}
      ...
      />
      <Text style={styles.titleText}>ชื่อผู้เล่น 1 </Text>
      ...
      <Text style={styles.titleText}>ชื่อผู้เล่น 2 </Text>
      ...
      <Text style={styles.titleText}>ชื่อคู่แข่ง 1 </Text>
      ...
      <View style={styles.pickerContainer}>
        ...
        >
          {filteredUsers3.map((user, index) => (
            <Picker.Item key={index} label={user} value={user} />
          ))}
        </Picker>
      </View>
      <Text style={styles.titleText}>ชื่อคู่แข่ง 2 </Text>
      ...
      <View style={styles.pickerContainer}>
        ...
        >
          {filteredUsers4.map((user, index) => (
            <Picker.Item key={index} label={user} value={user} />
          ))}
        </Picker>
      </View>
      <Text style={styles.titleText}>เซต: </Text>
      <View style={styles.pickerContainer}>
        <Picker
          selectedValue={selectedSet}
          style={styles.pickerStyle}
          onValueChange={(itemValue, itemIndex) => setSelectedSet(itemValue)}
        >
          <Picker.Item label="เซตที่ 1" value="set1" />
          <Picker.Item label="เซตที่ 2" value="set2" />
          <Picker.Item label="เซตที่ 3" value="set3" />
        </Picker>
      </View>

      <Text style={styles.titleText}>คะแนน: </Text>
      <TextInput
        style={styles.input}
